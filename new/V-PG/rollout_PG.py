...
model = load_model("PG_model.h5", custom_objects={'custom_loss': my_custom_loss})


# Action function to choose the best action given the q-function if not exploring based on epsilon
def choose_action(state, allowed_actions, action_space, state_size):
    n = 0
    allowed_act_prob = np.zeros((1, len(action_space)))
    ...
    actions = np.ones((1, len(action_space)))
    rewards = np.ones((1, 1))
    state = np.array(state).reshape(1, state_size)
    pred = model.predict([state, actions, rewards, allowed_act_prob])
